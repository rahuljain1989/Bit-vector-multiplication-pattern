...
the theory of quantifier-free foxed-width
...
We know that $a * b$ is commutative and, in a pattern matching
...
if more than two bits are available that are yet to be added
and half adders are used if there are only two bits that are left to be added.
%
The adders generate carry bits are aligned to $(o+1)$th output bit.
%
The carry bits are added to the partial products for $(o+1)th$ bit
...
%
For a hardware design containing multiplication, it is not
trivial to verify that the design indeed implements the
